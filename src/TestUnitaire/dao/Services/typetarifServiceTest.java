...
typetarifService ts = new typetarifService() ;
    ...
    public void testFindByObject() {
        //tester la methode findbyObject sur la base de données
        typetarif tp = new typetarif("daily",22);
        typetarif typT = ts.findByObject(tp) ;
        assertEquals("daily" , typT.getTypetarif());//tester l'egalite entre le type donné et le Typetarif sur La base de donnes
    }
}